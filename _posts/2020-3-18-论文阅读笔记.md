...
$$
$y'$ 表示第i个固有的特征图，$ y' \in Y' $ ,$$\Phi  _{i,j}$$ 表示生成第$$i$$ 个固有特征的第$$j$$ 个（最后一个除外） __ghost feature__ 的线性函数.也就是说$y_i'$ 能够有一个或者多个 __ghost feature maps__  $$\{y _ {ij}\} _ {j=1}^{s}$$ ，$\Phi{i,s}$ 是每个固有特征的__identity mapping__ (也就是不做任何变换，保持原有特征) .由此可知:$ n = m \cdot s$ ,得到的n个特征图为$$Y = [y_{11},y_{12},...,y_{ms}] $$ ，整个流程如图2所示：
